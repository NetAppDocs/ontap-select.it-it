...
Il traffico di rete del cluster deve fluire attraverso una rete di livello 2 a bassa latenza e non instradata. A causa dei requisiti di throughput e latenza del cluster, si prevede che il cluster ONTAP Select sia fisicamente ubicato in prossimità (ad esempio, multipack, singolo data center).  Non è supportata la creazione di configurazioni di cluster estesi a quattro, sei, otto, dieci o dodici nodi separando i nodi HA su una WAN o su distanze geografiche significative. È supportata una configurazione estesa a due nodi con un mediatore.
