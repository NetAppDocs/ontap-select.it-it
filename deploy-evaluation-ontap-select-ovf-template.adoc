...
summary: 'È possibile distribuire un"istanza di valutazione di 90 giorni di un cluster ONTAP Select a nodo singolo.' 
---
...
[role="lead"]
È possibile distribuire un'istanza di valutazione di 90 giorni di un cluster ONTAP Select di dimensioni variabili da un singolo nodo fino a 99 nodi utilizzando un modello OVF su VMware ESXi. Puoi scaricare una licenza di valutazione se sei un cliente nuovo o esistente. Ad esempio, sei un cliente esistente e vuoi provare una nuova configurazione prima di acquistare una licenza.
...
* È necessario installare il modello OVF (contenuto in un file ova) utilizzando il client autonomo vSphere o il client Web vSphere (l'unica opzione per ESXi 8.0 e versioni successive). Non è necessario utilizzare l'utility di amministrazione ONTAP Select Deploy.




== Fase 1: preparare l'host del cluster ONTAP Select
...
Per ulteriori informazioni sulle opzioni di configurazione dell'host, fare riferimento al link:esxi-host-configuration-and-preparation-checklist.html["Documentazione di installazione di ONTAP Select"].



== Passaggio 2: distribuire un cluster ONTAP Select a nodo singolo utilizzando un modello OVF
...
** Seleziona *Datacenter*. Quindi fai clic con il pulsante destro del mouse e seleziona *Distribuisci modello OVF*.
