...
* Esempio di valore Se si seleziona _Esempio di valore_ in una chiamata API, viene visualizzata una tipica struttura JSON per la chiamata. È possibile modificare l'esempio secondo necessità e utilizzarlo come input per la richiesta.
* Modello Se si seleziona _Modello_, viene visualizzato un elenco completo dei parametri JSON, con una descrizione per ciascun parametro.
