...
Cluster ONTAP Select:: È possibile creare un cluster _ONTAP Select_ composto da uno, due, quattro, sei, otto, dieci o dodici nodi. I cluster multi-nodo contengono sempre una o più coppie HA. Ad esempio, un cluster a quattro nodi è costituito da due coppie HA. Un cluster a nodo singolo non offre funzionalità HA.
...
Per maggiori informazioni, fare riferimento alle due sezioni link:concept_workflow_deploy.html["Pianificare"] e link:concept_lic_evaluation.html["Licenza"].
...
Archivio di credenziali:: L'archivio credenziali di Deploy è un database sicuro che contiene le credenziali dell'account. Viene utilizzato principalmente per registrare gli host hypervisor come parte della creazione di un nuovo cluster. Consultare la sezione link:concept_workflow_deploy.html["Pianificare"] per ulteriori informazioni.
