...
Per ulteriori best practice e altri requisiti, fare riferimento alle sezioni link:concept_ha_config.html#two-node-ha-versus-multi-node-ha["Ha a due nodi rispetto a ha a più nodi"] e link:reference_plan_best_practices.html#two-node-stretched-ha-metrocluster-sds-best-practices["Best practice di ha (MetroCluster SDS) con due nodi estesi"].
