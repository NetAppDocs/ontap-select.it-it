...
Per ulteriori informazioni sulla preparazione del server Linux, inclusa la configurazione richiesta e i pacchetti software, fare riferimento a link:kvm-host-configuration-and-preparation-checklist.html["lista di controllo della configurazione host"]. Consultare i requisiti dell'hypervisor per le distribuzioni Linux attualmente supportate.
...
Fare riferimento alle informazioni di pianificazione specifiche dell'hypervisor e link:reference_release_notes.html["note di rilascio"] per ulteriori dettagli sulle piattaforme supportate.
