...
. Seleziona image:icon_kebab.gif["Opzioni"] a destra del cluster e seleziona *Porta offline*.
+
Se l'opzione offline non è disponibile, il cluster è già in stato offline.

. Seleziona *Sì* nella finestra a comparsa per confermare la richiesta.
. Seleziona *Aggiorna* di tanto in tanto per verificare che il cluster sia offline.
. Per rimettere online il cluster, selezionare image:icon_kebab.gif["Opzioni"] e selezionare *Riporta online*.
. Seleziona *Aggiorna* di tanto in tanto per verificare che il cluster sia online.
...
. Accedere all'interfaccia utente Web dell'utilità di implementazione utilizzando l'account amministratore.
. Selezionare la scheda *cluster* nella parte superiore della pagina e selezionare il cluster desiderato dall'elenco.
. Seleziona image:icon_kebab.gif["Opzioni"] a destra del cluster e seleziona *Elimina*.
+
Se l'opzione di eliminazione non è disponibile, il cluster non si trova in uno stato offline.

. Seleziona *Aggiorna* di tanto in tanto per confermare che il cluster sia stato rimosso dall'elenco.
...
.Fasi
. Accedere all'interfaccia utente Web dell'utilità di implementazione utilizzando l'account amministratore.
. Seleziona la scheda *Cluster* in alto a sinistra della pagina e seleziona il cluster desiderato dall'elenco.
. Seleziona image:icon_kebab.gif["Opzioni"] sul lato destro della pagina e scegli *Aggiorna cluster*.
