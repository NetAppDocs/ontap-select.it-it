...
. Seleziona la scheda *Amministrazione* nella parte superiore della pagina, quindi seleziona *Network Checker*.
. Seleziona *Avvia nuova esecuzione*, quindi seleziona gli host e le reti per la coppia HA
+
È possibile aggiungere e configurare coppie ha aggiuntive in base alle esigenze.

. Seleziona *Avvia* per iniziare il test di connettività di rete.
