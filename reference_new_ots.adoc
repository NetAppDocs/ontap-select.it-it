...
Espansione del cluster:: ONTAP Select 9.13.1 introduce l'espansione del cluster, che offre protezione dall'alta disponibilità (ha) in caso di impatto di un'interruzione del nodo sulla capacità di un cluster di erogare i dati. A partire da ONTAP Select 9.13.1, puoi espandere un cluster a nodo singolo in un cluster a due nodi configurando entrambi i nodi come coppia ha. La procedura viene automatizzata e avviata da ONTAP Select Deploy utilizzando l'interfaccia CLI o Web. Per eseguire questa procedura, è necessario soddisfare determinati requisiti hardware e software. link:concept_cluster_expansion.html["Scopri di più"].
