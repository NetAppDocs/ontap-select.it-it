...
== Passaggio 1: scaricare l'immagine della macchina virtuale
...
Verificare la firma ONTAP Select Open Virtualization Appliance (OVA) prima di installare il pacchetto di installazione.
...
== Passaggio 3: distribuire la macchina virtuale

Installare e avviare ONTAP Select Deploy VM utilizzando l'immagine OVF VM. Come parte del processo di installazione, è necessario configurare l'interfaccia di rete per utilizzare DHCP o una configurazione IP statica.
...
.. Inserisci la password e seleziona *Invio*.
.. Attendere la seguente richiesta:
+
