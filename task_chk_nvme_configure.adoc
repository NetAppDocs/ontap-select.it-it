...
* Per un host ESXi, ONTAP Select 9.7 o versione successiva con un'utilità di amministrazione Deploy supportata
* Per un host KVM, ONTAP Select 9.17.1 o versione successiva con un'utilità di amministrazione Deploy supportata
...
Segui le link:kvm-host-configuration-and-preparation-checklist.html["checklist per la preparazione dell'host"] e rivedi le informazioni richieste per il link:reference_chk_deploy_req_info.html["distribuire l'installazione dell'utilità"] e il link:reference_chk_select_req_info.html["Installazione ONTAP Select"] per ulteriori informazioni.
