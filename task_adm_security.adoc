...
. Seleziona l'icona a forma di omino in alto a destra della pagina, quindi seleziona *Cambia password*.
. Inserisci la password attuale e quella nuova quando richiesto e seleziona *Invia*.
...
. Selezionare *Server di gestione* > *Aggiungi vCenter*.
. Inserisci le seguenti informazioni e seleziona *Aggiungi*.
+
[cols="30,70"]
|===
...
|===
. Facoltativamente, selezionare image:icon_kebab.gif["Opzioni"] dopo aver aggiunto il nuovo server di gestione, quindi selezionare una delle seguenti opzioni:
+
