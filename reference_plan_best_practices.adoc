...
NOTE: Durante l'installazione su ESXi 8.0 o versioni successive, ONTAP Select utilizza il driver vNVMEe indipendentemente dal fatto che il disco di sistema risieda su un SSD o su un disco NVMe. Questo imposta il livello hardware della VM su 13, che è compatibile con ESXi 8.0 e versioni successive.
...
Per saperne di più su come configurare un Open vSwitch su un host hypervisor, fare riferimento al link:https://www.netapp.com/media/13134-tr4613.pdf["Architettura e Best practice del prodotto ONTAP Select su KVM"^] technical report.
