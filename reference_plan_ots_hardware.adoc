...
* KVM su Red Hat Enterprise Linux (RHEL) 10.1, 10.0, 9.7, 9.6, 9.5, 9.4, 9.2, 9.1, 9.0, 8.8, 8.7, e 8.6
* KVM su Rocky Linux 10.1, 10.0, 9.7, 9.6, 9.5, 9.4, 9.3, 9.2, 9.1, 9.0, 8.9, 8.8, 8.7, and 8.6
...
NOTE: ESXi 7.0 GA ha raggiunto la fine dello stato di disponibilità.  Se si dispone di cluster ONTAP Select con questa versione, è necessario eseguire l'aggiornamento alle versioni supportate come da https://mysupport.netapp.com/matrix["Tool di matrice di interoperabilità (IMT)"^] .
...
ONTAP Select supporta modelli di CPU basati su processori Intel Xeon Sandy Bridge e Advanced Micro Devices (AMD).

Consultare le  https://mysupport.netapp.com/matrix["Tool di matrice di interoperabilità"^] per ulteriori informazioni.
...
.Del processore
I microprocessori supportati includono i processori server Intel Xeon e i processori AMD.

Consultare link:https://www.intel.com/content/www/us/en/products/processors/xeon/view-all.html?Processor+Type=1003["Processori Intel Xeon"^] e link:https://www.amd.com/en/products/processors/server/epyc.html["CPU server AMD EPYC"^] per ulteriori informazioni.
...
| Cluster da quattro, sei, otto, dieci o dodici nodi | 2 x 10 GbE | 4 x 10 GbE o 2 x 25/40 GbE 
|===
...
Scopri di piùlink:concept_lic_platforms.html["le offerte di licenze della piattaforma per ONTAP Select"]
